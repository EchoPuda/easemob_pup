...
public class EasemobHandler {
    ...
    public static void sendTextMessage(MethodCall call, MethodChannel.Result result) {
        ...
        boolean chatType = (boolean)call.argument("chatType");
        if (chatType){
    ...
    public static void sendImageMessage(MethodCall call, MethodChannel.Result result) {
        ...
        EMMessage message = EMMessage.createImageSendMessage(imagePath, (boolean)call.argument("originally"), call.argument("toChatUsername"));
        //如果是群聊，设置chattype，默认是单聊
        boolean chatType = (boolean)call.argument("chatType");
        if (chatType){
            message.setChatType(EMMessage.ChatType.GroupChat);
